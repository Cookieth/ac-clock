...
for video in video_logs["video_urls_paths"]:
    if video_logs["video_urls_paths"][video][1] == "" or not os.path.isfile(video_logs["video_urls_paths"][video][1]):
        ...
        video_logs["video_urls_paths"][video][2] = yt_video.length
...
while True:
    ...
    seconds_in_video = video_logs["video_urls_paths"][hour_key][2]

    if seconds_in_video < target_seconds:
        # print("Playing hour {}...".format(hour))
        subprocess.run("afplay \"{}\"".format(video_logs["video_urls_paths"][hour_key][1]), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    else:
        # print("Playing hour {} for only {} seconds...".format(hour, target_seconds))
        subprocess.run("afplay \"{}\" -t {}".format(video_logs["video_urls_paths"][hour_key][1], target_seconds), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        subprocess.run("afplay \"{}\"".format(video_logs["bells"][1]), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

# This method uses IPC for multiple processes... much more complicated
# while True:
#     hour = datetime.now().hour
#     hour_key = str(int(hour / 10)) + str(int(hour % 10))
